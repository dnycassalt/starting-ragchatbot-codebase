...
class TestBasicResponseGeneration:
    ...
    def test_generate_response_with_conversation_history(
        ...
        with patch("anthropic.Anthropic", return_value=mock_anthropic_client):
            ...
            response = generator.generate_response(
                query="test question", conversation_history=history
            )
    ...
    def test_generate_response_without_conversation_history(
        ...
        with patch("anthropic.Anthropic", return_value=mock_anthropic_client):
            ...
            response = generator.generate_response(
                query="test question", conversation_history=None
            )
...
class TestToolCalling:
    ...
    def test_generate_response_with_tools_not_used(
        ...
        with patch("anthropic.Anthropic", return_value=mock_anthropic_client):
            ...
            response = generator.generate_response(
                query="What is 2+2?", tools=tool_defs
            )
...
class TestToolExecutionHandler:
    ...
    def test_handle_tool_execution_single_tool(
        ...
        with patch("anthropic.Anthropic", return_value=mock_anthropic_client):
            ...
            result = generator._handle_tool_execution(
                mock_anthropic_response_with_tool, base_params, mock_tool_manager
            )
    ...
    def test_handle_tool_execution_preserves_message_history(
        self,
        mock_anthropic_client,
        mock_anthropic_response_with_tool,
        mock_anthropic_final_response,
    ...
    def test_handle_tool_execution_with_tool_error(
        self,
        mock_anthropic_client,
        mock_anthropic_response_with_tool,
        mock_anthropic_final_response,
...
class TestAPIParameters:
    ...
    def test_api_params_with_tools(
        self, mock_anthropic_client, mock_anthropic_response_no_tool
    ):
...
class TestExceptionHandling:
    ...
    def test_generate_response_with_invalid_api_key(self, mock_anthropic_client):
        ...
        with patch("anthropic.Anthropic", return_value=mock_anthropic_client):
            generator = AIGenerator(
                api_key="invalid-key", model="claude-sonnet-4-20250514"
            )
    ...
    def test_generate_response_with_network_error(self, mock_anthropic_client):
        ...
        with patch("anthropic.Anthropic", return_value=mock_anthropic_client):
            ...
            mock_anthropic_client.messages.create.side_effect = ConnectionError(
                "Network unreachable"
            )
...
class TestSystemPrompt:
    ...
    def test_system_prompt_base_content(
        self, mock_anthropic_client, mock_anthropic_response_no_tool
    ):
    ...
    def test_system_prompt_with_history_appended(
        self, mock_anthropic_client, mock_anthropic_response_no_tool
    ):
