...
class TestQueryFlowWithoutTools:
    ...
    def test_query_without_session(self, test_config, mock_anthropic_client, mock_anthropic_response_no_tool):
                    ...
                    with patch('chromadb.utils.embedding_functions.SentenceTransformerEmbeddingFunction'):
                        with patch('document_processor.DocumentProcessor'):
                            with patch('session_manager.SessionManager'):
                                ...
                                response, sources = rag.query("What is 2+2?", session_id=None)
    ...
    def test_query_with_session(self, test_config, mock_anthropic_client, mock_anthropic_response_no_tool, mock_session_manager):
                ...
                with patch('chromadb.utils.embedding_functions.SentenceTransformerEmbeddingFunction'):
                    with patch('document_processor.DocumentProcessor'):
                        with patch('session_manager.SessionManager'):
                            ...
                            error_results = SearchResults.empty("Search error: ChromaDB failed")
