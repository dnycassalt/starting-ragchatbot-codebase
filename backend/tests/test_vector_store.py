...
class TestVectorStoreSearch:
    ...
    def test_search_without_filters(
        self, mock_chroma_client, sample_chroma_query_result, test_config
    ):
            ...
            with patch(
                ...
                with patch.object(
                    ...
                    results = store.search(
                        "test query", course_name="MCP", lesson_number=1
                    )
        ...
        with patch("chromadb.PersistentClient", return_value=mock_chroma_client):
            with patch(
                ...
                with patch.object(store, "_resolve_course_name", return_value=None):
                    results = store.search(
                        "test query", course_name="NonexistentCourse"
                    )
...
class TestDataAddition:
    ...
    def test_add_course_content(
        self, mock_chroma_client, sample_course_chunks, test_config
    ):
        ...
        with patch("chromadb.PersistentClient", return_value=mock_chroma_client):
            with patch(
                ...
                # Check first chunk
                assert (
                    call_args["metadatas"][0]["course_title"] == "Introduction to MCP"
                )
    ...
    def test_get_course_link_success(self, mock_chroma_client, test_config):
        ...
        with patch("chromadb.PersistentClient", return_value=mock_chroma_client):
            with patch(
                ...
                store.course_catalog.get.assert_called_once_with(
